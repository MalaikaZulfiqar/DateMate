    ...
    <Pressable style={styles.container} onPress={onPress}>
      ...
      <View style={styles.content}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 5 }}>
          <Text style={[style.title, { fontSize: 14 }]}>{name}</Text>
          <Text style={[style.title, { fontSize: 14 }]}>{rate}</Text>
        </View>
        <View style={{ marginBottom: 5 }}>
          <Text style={[style.title, { textAlign: 'left', fontSize: 12, color: colors.gray }]}>{location}</Text>
        </View>
        <View style={{ flexDirection: 'row', fontSize: 14 }}>
          <Text style={[style.title, { color: colors.primaryColor, fontSize: 14 }]}>Booking Price:</Text>
